...
class MaGiamGiaController extends Controller {
    ...
    {
        $this->maGiamGiaModel = new \App\Models\Admin\MaGiamGiaModel();
    }
    ...
    public function index()
    {
        ...
        $voucher_list = $this->maGiamGiaModel->getAllVouchers($filters, $limit, $offset);
        ...
        $total = $this->maGiamGiaModel->countAllVouchers($filters);
        $stats = $this->maGiamGiaModel->getThongKe();

        $data = [
            'tieu_de' => 'Quản lý mã giảm giá - Chuỗi Ngọc Phong Thủy',
        ...
        $this->view('admin_ma_giam_gia', $data, 'admin');
    }

    public function them()
    {
        $data = [
            'tieu_de' => 'Thêm mới mã giảm giá',
            'current_page' => 'voucher',
            'is_edit' => false,
            'voucher' => null,
        ...
        $this->view('admin_ma_giam_gia_form', $data, 'admin');
    }

    public function sua($id)
    {
        $voucher = $this->maGiamGiaModel->getVoucherById($id);
        if (!$voucher) {
            header('Location: ' . APP_URL . '/admin/ma-giam-gia');
            exit;
        }

        $san_pham_list = $this->maGiamGiaModel->getVoucherProducts($id);
        ...
        $data = [
            'tieu_de' => 'Chỉnh sửa mã giảm giá: ' . $voucher['ma_voucher'],
            'current_page' => 'voucher',
            'is_edit' => true,
            'voucher' => $voucher,
            'hang_thanh_vien_list' => (new \App\Models\Admin\HangThanhVienModel())->layTatCa(),
            'danh_muc_list' => (new \App\Models\Admin\DanhMucModel())->layTatCa(),
            'voucher_danh_muc' => $this->maGiamGiaModel->getVoucherCategories($id),
    ...
    public function store()
    {
        ...
        try {
            ...
            $this->maGiamGiaModel->createVoucher($data);
    ...
    public function update($id)
    {
        ...
        try {
            ...
            $this->maGiamGiaModel->updateVoucher($id, $data);
    ...
    public function xoa()
    {
        ...
        try {
            ...
            $id = $input['id'] ?? null;
            if (!$id) throw new \Exception('Thiếu ID mã giảm giá');

            $this->maGiamGiaModel->deleteVoucher($id);
    ...
    public function toggle_status()
    {
        ...
        try {
            ...
            $this->maGiamGiaModel->toggleStatus($id, $status);
    ...
    /**
     * API: Kiểm tra mã giảm giá và tính giảm giá
     * POST /admin/ma-giam-gia/api/check
    ...
    public function apiCheckVoucher()
    {
        ...
        try {
            ...
            if (empty($ma)) {
                echo json_encode(['success' => false, 'message' => 'Vui lòng nhập mã giảm giá.'], JSON_UNESCAPED_UNICODE);
                return;
            }

            $result = $this->maGiamGiaModel->checkVoucherByCode($ma, $tongTien);
