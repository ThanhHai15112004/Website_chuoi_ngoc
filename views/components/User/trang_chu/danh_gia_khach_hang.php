    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <?php if (!empty($danh_gia)): ?>
                <?php foreach ($danh_gia as $index => $dg): ?>
                <div class="p-8 rounded-2xl relative" style="background: #FAF7F2;" data-aos="fade-up" data-aos-delay="<?= $index * 100 ?>">
                    <!-- Stars -->
                    <div class="flex gap-0.5 mb-4">
                        <?php for($i=1; $i<=5; $i++): ?>
                        <svg class="w-5 h-5" style="color: <?= $i <= $dg['so_sao'] ? '#d4af37' : '#ddd' ?>;" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                        <?php endfor; ?>
                    </div>

                    <p class="italic mb-6 leading-relaxed" style="color: #444;">"<?= htmlspecialchars($dg['noi_dung']) ?>"</p>
                    
                    <div class="flex items-center">
                        <div class="w-12 h-12 rounded-full overflow-hidden mr-4 border-2" style="border-color: rgba(212,175,55,0.3);">
                            <?php if (!empty($dg['avatar_khach'])): ?>
                                <img src="<?= APP_URL . '/' . ltrim($dg['avatar_khach'], '/') ?>" alt="<?= htmlspecialchars($dg['ten_khach']) ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                                <img src="https://ui-avatars.com/api/?name=<?= urlencode($dg['ten_khach']) ?>&background=random&color=fff&size=96" alt="<?= htmlspecialchars($dg['ten_khach']) ?>" class="w-full h-full object-cover">
                            <?php endif; ?>
                        </div>
                        <div>
                            <h4 class="font-semibold" style="color: #111;"><?= htmlspecialchars($dg['ten_khach']) ?></h4>
                            <p class="text-xs" style="color: #999;"><?= htmlspecialchars($dg['hang_thanh_vien'] ?? 'Khách hàng') ?></p>
                        </div>
                    </div>
            ...
            <?php else: ?>
                <div class="col-span-1 md:col-span-3 text-center py-8 text-gray-500">
                    Chưa có đánh giá nào được nổi bật.
                </div>
            <?php endif; ?>
        </div>
    </div>
