...
<div id="form-tu-van" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 relative overflow-hidden">
    ...
    <div class="pl-4">
        <h2 class="text-2xl font-bold text-gray-900 mb-2">Gửi yêu cầu tư vấn</h2>
...
<script>
    async function handleContactSubmit(e) {
        ...
        try {
            const response = await fetch('<?= APP_URL ?>/lien-he/gui', {
                method: 'POST',
                headers: {
            ...
            if (result.success) {
                ...
                document.querySelector('#contactSuccessMessage p').textContent = result.message || 'Cảm ơn bạn đã liên hệ! Chúng tôi sẽ phản hồi sớm nhất.';
            } else {
                errorMsg.classList.remove('hidden');
                errorMsg.querySelector('p').textContent = result.message || 'Có lỗi xảy ra, vui lòng thử lại.';
            }
        } catch (error) {
            errorMsg.classList.remove('hidden');
            errorMsg.querySelector('p').textContent = 'Lỗi kết nối, vui lòng kiểm tra lại mạng.';
        } finally {
