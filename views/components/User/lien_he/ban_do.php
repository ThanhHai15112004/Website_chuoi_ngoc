<!-- views/components/User/lien_he/ban_do.php -->
<section class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Tìm chúng tôi trên bản đồ</h2>
            <p class="text-gray-600 mt-1">Ghé thăm cửa hàng để trải nghiệm trực tiếp các sản phẩm ngọc và đá quý thiên nhiên.</p>
        </div>
        <?php 
            $map_address = implode(', ', array_filter([$cau_hinh['dia_chi_chi_tiet'] ?? '', $cau_hinh['phuong_xa'] ?? '', $cau_hinh['quan_huyen'] ?? '', $cau_hinh['tinh_thanh'] ?? '']));
            $map_url = 'https://maps.google.com/?q=' . urlencode($map_address);
        ?>
        <a href="<?= htmlspecialchars($map_url) ?>" target="_blank" class="inline-flex items-center px-5 py-2.5 border border-red-800 text-sm font-medium rounded-full text-red-800 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-800 transition duration-300 flex-shrink-0">
    ...
    <div class="rounded-xl overflow-hidden shadow-inner border border-gray-200 h-[300px] md:h-[450px] relative bg-gray-50 flex items-center justify-center">
        <?php if (!empty($cau_hinh['google_map_iframe'])): ?>
            <!-- Google Maps iframe from config -->
            <div class="w-full h-full [&>iframe]:w-full [&>iframe]:h-full">
                <?= $cau_hinh['google_map_iframe'] ?>
            </div>
        <?php else: ?>
