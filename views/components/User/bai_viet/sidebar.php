<aside class="space-y-8">
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div class="space-y-5">
            <?php 
            $sidebar_popular = !empty($popular_articles) ? $popular_articles : (!empty($recent_articles) ? array_slice($recent_articles, 0, 4) : []);
            foreach ($sidebar_popular as $index => $article): 
            ?>
            <a href="<?= APP_URL ?>/chi-tiet-bai-viet?slug=<?= htmlspecialchars($article['slug']) ?>" class="flex gap-4 group">
                <div class="w-20 h-20 shrink-0 overflow-hidden rounded-lg">
                    <img src="<?= $article['image'] ?>" alt="<?= htmlspecialchars($article['title']) ?>" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" onerror="this.onerror=null;this.src='<?= APP_URL ?>/images/Logo_.jpg';">
                </div>
    ...
    <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div class="flex flex-wrap gap-2">
            <?php
            $sidebar_tags = !empty($tags) ? $tags : ['Vòng ngọc', 'Đá phong thủy', 'Mệnh Kim', 'Tài lộc', 'Bình an', 'Tình duyên', 'Quà tặng mẹ', 'Trầm hương', 'Thạch anh'];
            foreach ($sidebar_tags as $tag):
            ?>
            <a href="<?= APP_URL ?>/bai-viet?q=<?= urlencode($tag) ?>" class="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded text-sm text-gray-600 hover:bg-red-50 hover:text-red-800 hover:border-red-200 transition-colors">
    ...
    <a href="<?= APP_URL ?>/vong-theo-menh" class="block overflow-hidden rounded-xl relative group">
        <img src="<?= APP_URL ?>/images/Sản phẩm/Vòng Ngọc/Sâm Panh Thuần/sam-panh-thuan-1.jpg" alt="Tìm vòng theo mệnh" class="w-full h-64 object-cover transition-transform duration-700 group-hover:scale-105">
