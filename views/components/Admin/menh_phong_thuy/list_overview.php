    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        <?php foreach ($destinies as $d): ?>
            <a href="<?= APP_URL ?>/admin/menh-phong-thuy/sua/<?= $d['id'] ?>" class="block bg-white rounded-xl shadow-sm border border-gray-100 p-4 hover:border-[#6B0D18] hover:shadow-md transition-all group relative">
                ...
                <div class="flex items-center gap-2 mb-3">
                    <span class="w-3 h-3 rounded-full" style="background-color: <?= $d['mau_dai_dien_hex'] ?? '#E5E7EB' ?>; box-shadow: 0 0 0 1px rgba(0,0,0,0.1)"></span>
                    <h4 class="font-bold text-gray-800 group-hover:text-[#6B0D18] transition-colors"><?= $d['ten_menh'] ?></h4>
                </div>
