...
<div class="bg-white rounded-[20px] shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50/50">
        ...
        <button onclick="openDrawer('drawerShipping')" class="px-4 py-2 bg-[#6B0D18] text-white text-sm font-medium rounded-lg hover:bg-red-900 transition-colors shadow-sm flex items-center gap-1">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-100" id="shipping-sortable">
                <?php foreach($shipping_methods as $index => $ship): ?>
                <tr class="hover:bg-gray-50 transition-colors group cursor-move <?= !$ship['status'] ? 'opacity-75' : '' ?>">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 shrink-0">
                                <span class="iconify text-xl" data-icon="<?= $ship['id'] == 'STORE' ? 'mdi:store' : 'mdi:truck-fast-outline' ?>"></span>
                            </div>
                            <div>
                                <span class="font-bold text-gray-900 text-sm whitespace-nowrap block"><?= $ship['name'] ?></span>
                                <span class="text-xs text-gray-500"><?= $ship['desc'] ?></span>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-900 font-medium mb-1"><span class="iconify inline-block text-gray-400" data-icon="mdi:map-marker"></span> <?= $ship['zone'] ?></div>
                        <div class="text-xs text-gray-500"><span class="iconify inline-block" data-icon="mdi:clock-outline"></span> <?= $ship['time'] ?></div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm font-bold <?= $ship['fee'] > 0 ? 'text-[#6B0D18]' : 'text-emerald-600' ?> mb-1">
                            <?= $ship['fee'] == 0 ? 'Miễn phí' : number_format($ship['fee'],0,',','.') . 'đ' ?>
                        </div>
                        <?php if($ship['freeship'] > 0): ?>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700">
                                Freeship từ <?= number_format($ship['freeship']/1000,0,',','.') ?>K
                            </span>
                        <?php endif; ?>
                    </td>
                    <td class="px-6 py-4 text-center">
                        <label class="inline-flex items-center cursor-pointer">
                            <div class="relative">
                                <input type="checkbox" class="sr-only toggle-switch" <?= $ship['status'] ? 'checked' : '' ?> onchange="markUnsaved()">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button onclick="openDrawer('drawerShipping')" class="w-8 h-8 rounded-lg text-blue-600 hover:bg-blue-50 flex items-center justify-center transition-colors tooltip" title="Sửa">
